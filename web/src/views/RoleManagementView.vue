<template>
  <div class="px-4 lg:px-8 py-6 space-y-5">
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-100">Role Management</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-slate-500">{{ store.roles.length }} roles defined</p>
      </div>
      <button
        @click="openModal()"
    ...
    <div v-else-if="!store.roles.length"
      ...
      <p class="text-sm text-gray-500 dark:text-slate-400 font-medium">No roles yet</p>
      <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">Create your first role to manage permissions</p>
    </div>
      ...
      >
        ...
        <div class="flex items-start justify-between px-6 py-4 border-b border-gray-100 dark:border-slate-700/60">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-gray-500 dark:text-slate-500 mt-0.5">{{ role.description || 'No description' }} · {{ role.userCount || 0 }} user{{ role.userCount !== 1 ? 's' : '' }}</p>
            </div>
          </div>
        ...
        <div class="px-6 py-4 overflow-x-auto">
          <table class="w-full text-xs">
            <thead>
              <tr>
                <th class="text-left text-gray-400 dark:text-slate-500 font-semibold uppercase tracking-wider pb-2 pr-4 w-32">Menu</th>
            ...
            <tbody class="divide-y divide-gray-50 dark:divide-slate-800">
              <tr v-for="menu in MENUS" :key="menu.key">
                ...
                <td v-for="action in ACTIONS" :key="action" class="py-2 px-3 text-center">
                  <span v-if="role.permissions?.[menu.key]?.[action]"
      ...
      <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0" enter-to-class="opacity-100"
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" @click.self="showModal = false">
          <Transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 scale-95" enter-to-class="opacity-100 scale-100">
            <div v-if="showModal" class="w-full max-w-lg bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-slate-700 overflow-hidden">

              <!-- Modal header -->
              <div class="flex items-center justify-between px-6 py-5 border-b border-gray-100 dark:border-slate-700">
                <h3 class="text-base font-bold text-gray-900 dark:text-slate-100">{{ editTarget ? 'Edit Role' : 'Add Role' }}</h3>
              ...
              <form @submit.prevent="handleSubmit" class="px-6 py-5 space-y-5">
                ...
                <div class="grid grid-cols-2 gap-3">
                  <div>
                    <label class="block text-sm font-semibold text-gray-700 dark:text-slate-300 mb-1.5">Role Name <span class="text-red-500">*</span></label>
                    <input v-model="form.name" required type="text" placeholder="e.g. Admin"
                ...
                <div>
                  <div class="flex items-center justify-between mb-3">
                    <label class="text-sm font-semibold text-gray-700 dark:text-slate-300">Permissions</label>
                    <div class="flex gap-2">
                      ...
                      <button type="button" @click="selectAll(false)"
                        class="text-xs text-gray-500 dark:text-slate-400 hover:underline">Clear all</button>
                    </div>
                  </div>
                    ...
                    <table class="w-full text-sm">
                      <thead>
                        <tr class="bg-gray-50 dark:bg-slate-800/60 border-b border-gray-200 dark:border-slate-700">
                          <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wider">Menu</th>
                          <th v-for="action in ACTIONS" :key="action" class="px-4 py-3 text-center">
                            <div class="flex flex-col items-center gap-1.5">
                              <label class="inline-flex items-center justify-center cursor-pointer" :title="`Toggle all ${action}`">
                                <input type="checkbox" :checked="allActionChecked(action)"
                                  @change="toggleAction(action)" class="sr-only peer" />
                                <div class="w-5 h-5 rounded-md border-2 transition-all duration-150 flex items-center justify-center
                                  ...
                                  <svg class="w-3 h-3 text-white transition-all" :class="allActionChecked(action) ? 'opacity-100 scale-100' : 'opacity-0 scale-0'"
                              ...
                              <span class="text-xs font-semibold uppercase tracking-wider"
                                :class="allActionChecked(action) ? 'text-blue-600 dark:text-blue-400' : 'text-gray-400 dark:text-slate-500'">
                                {{ action }}
                              </span>
                            </div>
                          </th>
                        ...
                        <tr v-for="menu in MENUS" :key="menu.key"
                          ...
                          <td v-for="action in ACTIONS" :key="action" class="px-4 py-3 text-center">
                            <label class="inline-flex items-center justify-center cursor-pointer">
                              <input type="checkbox"
                                :checked="form.permissions[menu.key]?.[action] || false"
                                @change="togglePerm(menu.key, action, $event.target.checked)"
                                class="sr-only peer" />
                              <div class="w-5 h-5 rounded-md border-2 transition-all duration-150 flex items-center justify-center
                                peer-checked:bg-blue-600 peer-checked:border-blue-600
                                border-gray-300 dark:border-slate-600 bg-white dark:bg-slate-800
                                ...
                                <svg class="w-3 h-3 text-white transition-all"
                                  :class="form.permissions[menu.key]?.[action] ? 'opacity-100 scale-100' : 'opacity-0 scale-0'"
                                  fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7" />
                                </svg>
                              </div>
                ...
                <div class="flex gap-3 pt-1">
                  <button type="button" @click="showModal = false"
                    class="flex-1 py-2.5 text-sm font-semibold text-gray-600 dark:text-slate-400 border border-gray-300 dark:border-slate-600 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors">
                    Cancel
                  </button>
                  <button type="submit" :disabled="submitting"
                    class="flex-1 py-2.5 text-sm font-bold text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-60 rounded-xl transition-colors flex items-center justify-center gap-2">
        ...
        <div v-if="deleteTarget" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" @click.self="deleteTarget = null">
          <div class="w-full max-w-sm bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-slate-700 p-6 space-y-4">
            ...
            <div class="text-center">
              <h3 class="text-base font-bold text-gray-900 dark:text-slate-100">Delete Role</h3>
              <p class="mt-1 text-sm text-gray-500 dark:text-slate-400">
                Delete role <span class="font-semibold text-gray-700 dark:text-slate-300">"{{ deleteTarget.name }}"</span>?
            ...
            <div class="flex gap-3">
              ...
              <button @click="handleDelete" :disabled="deleting"
                class="flex-1 py-2.5 text-sm font-bold text-white bg-red-600 hover:bg-red-700 disabled:opacity-60 rounded-xl transition-colors flex items-center justify-center gap-2">
...
const MENUS = [
  { key: 'dashboard', label: 'Dashboard', d: 'M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z' },
  { key: 'urls',      label: 'URLs',      d: 'M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14' },
  { key: 'users',     label: 'Users',     d: 'M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z' },
  ...
  { key: 'api_key',   label: 'API Key',   d: 'M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z' },
  { key: 'api_keys',  label: 'API Key Manage', d: 'M9 3H5a2 2 0 00-2 2v4m6-6h10a2 2 0 012 2v4M9 3v18m0 0h10a2 2 0 002-2V9M9 21H5a2 2 0 01-2-2V9m0 0h18' },
  { key: 'roles',     label: 'Roles',     d: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z' },
];
...
const emptyPermissions = () =>
  Object.fromEntries(MENUS.map((m) => [m.key, { view: false, edit: false, delete: false }]));
...
const selectAll = (val) => {
  for (const m of MENUS) {
    for (const a of ACTIONS) form.value.permissions[m.key][a] = val;
  }
};

const allActionChecked = (action) => MENUS.every((m) => form.value.permissions[m.key]?.[action]);
const allMenuChecked = (menuKey) => ACTIONS.every((a) => form.value.permissions[menuKey]?.[a]);

const toggleAction = (action) => {
  const all = allActionChecked(action);
  for (const m of MENUS) form.value.permissions[m.key][action] = !all;
};

const toggleMenu = (menuKey, val) => {
  for (const a of ACTIONS) form.value.permissions[menuKey][a] = val;
};
