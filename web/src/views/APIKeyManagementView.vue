<template>
  <div class="px-4 lg:px-8 py-6">
    ...
    <div class="flex items-center justify-between mb-4">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-slate-100">API Key Management</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-slate-500">{{ store.pagination.total }} keys total</p>
      </div>
      <button
        @click="openCreate"
        class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold px-4 py-2.5 rounded-xl transition-colors shadow-sm shadow-blue-500/25"
      >
    ...
    <div class="flex items-start gap-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/40 rounded-2xl px-4 py-3.5 mb-4">
      ...
      <p class="text-sm text-blue-700 dark:text-blue-300">
        Authenticate requests by adding
        <code class="font-mono bg-blue-100 dark:bg-blue-800/50 px-1.5 py-0.5 rounded text-xs">X-API-Key: &lt;key&gt;</code>
        to your request headers. Keys are shown only once on creation.
      </p>
    </div>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-2xl border border-gray-200 dark:border-slate-700/60 shadow-sm overflow-hidden">
        ...
        <div class="w-44 shrink-0">
          <AppSelect
            ...
            :options="[
              ...
              { value: 'revoked',  label: 'Revoked',  dot: 'bg-red-500' },
            ]"
      ...
      <div v-else-if="!store.keys.length" class="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p class="text-sm font-medium text-gray-500 dark:text-slate-400">No API keys found</p>
        <p class="text-xs text-gray-400 dark:text-slate-500 mt-1">Create a new key to get started</p>
      </div>
        ...
        <table class="w-full text-sm">
          <thead>
            <tr class="border-b border-gray-100 dark:border-slate-800 bg-gray-50 dark:bg-slate-800/50">
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-500 uppercase tracking-wide">Key</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-500 uppercase tracking-wide">API Key</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-500 uppercase tracking-wide">User</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-500 uppercase tracking-wide">Scopes</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-500 uppercase tracking-wide">Status</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-500 uppercase tracking-wide">Expires</th>
              <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 dark:text-slate-500 uppercase tracking-wide">Last Used</th>
              ...
              <td class="px-4 py-3.5">
                <div class="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    :title="copiedId === key.id ? 'Copied!' : 'Copy full key'"
                  >
      ...
      <!-- Pagination -->
      <div v-if="store.pagination.pages > 1" class="flex items-center justify-between px-5 py-3.5 border-t border-gray-100 dark:border-slate-800">
        <p class="text-xs text-gray-500 dark:text-slate-500">
          {{ (store.pagination.page - 1) * store.pagination.limit + 1 }}–{{ Math.min(store.pagination.page * store.pagination.limit, store.pagination.total) }}
    ...
    <Teleport to="body">
      <div v-if="showModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="closeModal">
        <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-md border border-gray-100 dark:border-slate-700 overflow-hidden">
          <div class="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100 dark:border-slate-800">
            <h3 class="text-base font-bold text-gray-900 dark:text-slate-100">{{ editing ? 'Edit API Key' : 'New API Key' }}</h3>
          ...
          <div class="px-6 py-5 space-y-4">
            <!-- Key Name -->
            <div>
              <label class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1.5">Key Name <span class="text-red-500">*</span></label>
              <input
            ...
            <div v-if="!editing" class="relative" ref="userDropdownRef">
              <label class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1.5">User</label>
              ...
              >
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <p v-else class="text-sm text-gray-400 dark:text-slate-500">My account (self)</p>
                ...
                <div v-if="userDropdownOpen" class="absolute z-50 left-0 right-0 mt-1.5 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl shadow-xl overflow-hidden">
                  ...
                  <div class="max-h-52 overflow-y-auto">
                    ...
                    >
                      ...
                      <span class="text-sm font-medium text-gray-700 dark:text-slate-300">My account (self)</span>
            ...
            <!-- Scopes -->
            <div>
              <label class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-2">Scopes</label>
            ...
            <!-- Status (edit only) -->
            <div v-if="editing">
              <label class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1.5">Status</label>
              <AppSelect
                v-model="form.status"
                :options="[
                  { value: 'active',   label: 'Active',   dot: 'bg-emerald-500' },
            ...
            <!-- Expires At -->
            <div>
              <label class="block text-xs font-semibold text-gray-700 dark:text-slate-300 mb-1.5">Expiry Date <span class="text-gray-400 dark:text-slate-500 font-normal">(optional)</span></label>
              <input
          ...
          <div class="px-6 pb-6 flex gap-3">
            <button @click="closeModal" class="flex-1 py-2.5 rounded-xl border border-gray-300 dark:border-slate-600 text-sm font-semibold text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors">Cancel</button>
            <button
              @click="handleSave"
              :disabled="saving"
              class="flex-1 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold transition-colors disabled:opacity-60 flex items-center justify-center gap-2"
            >
          ...
          <div class="bg-white dark:bg-slate-900 px-6 py-4 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between shrink-0">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-xs text-gray-400 dark:text-slate-500 mt-0.5">Key Analytics — owner: {{ statsModal.key?.user?.email }}</p>
              </div>
            </div>
          ...
          <div class="overflow-y-auto flex-1 p-5 space-y-4">
            ...
            <template v-else-if="statsModal.data">
              ...
              <div class="grid grid-cols-3 gap-3">
                <div v-for="card in [
                  { label: 'Total Links', value: statsModal.data.stats.totalLinks, color: 'from-emerald-500 to-teal-500', icon: 'M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1' },
                  { label: 'Active Links', value: statsModal.data.stats.activeLinks, color: 'from-blue-500 to-indigo-500', icon: 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z' },
                  { label: 'Total Clicks', value: statsModal.data.stats.totalClicks, color: 'from-violet-500 to-purple-500', icon: 'M15 15l-2 5L9 9l11 4-5 2zm0 0l5 5M7.188 2.239l.777 2.897M5.136 7.965l-2.898-.777M13.95 4.05l-2.122 2.122m-5.657 5.656l-2.12 2.122' },
                ]" :key="card.label"
              ...
              <div class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700/60 p-5">
                <div class="flex items-center justify-between mb-4">
                  <h4 class="text-sm font-bold text-gray-900 dark:text-slate-100">Engagement Timeline</h4>
                  <span class="text-xs text-gray-400 dark:text-slate-500">last 7 days</span>
                </div>
                <div class="h-40">
                  <Bar v-if="statsChartData" :data="statsChartData" :options="chartOptions" />
                  <div v-else class="h-full flex items-center justify-center text-sm text-gray-400 dark:text-slate-600">No data</div>
                </div>
              </div>
              ...
              <div v-if="statsModal.data.devices?.length" class="bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-700/60 p-5">
                <h4 class="text-sm font-bold text-gray-900 dark:text-slate-100 mb-4">Traffic by Device</h4>
      ...
      <div v-if="deleteTarget" class="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" @click.self="deleteTarget = null">
        <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-sm border border-gray-100 dark:border-slate-700">
          <div class="px-6 pt-6 pb-4 text-center">
            ...
            <h3 class="text-base font-bold text-gray-900 dark:text-slate-100">Delete API Key</h3>
            <p class="mt-2 text-sm text-gray-500 dark:text-slate-400">
              Delete <span class="font-semibold text-gray-700 dark:text-slate-300">{{ deleteTarget.keyName }}</span>?
              This action cannot be undone.
            </p>
          </div>
          <div class="px-6 pb-6 flex gap-3">
            <button @click="deleteTarget = null" class="flex-1 py-2.5 rounded-xl border border-gray-300 dark:border-slate-600 text-sm font-semibold text-gray-700 dark:text-slate-300 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors">Cancel</button>
            <button
              @click="handleDelete"
              :disabled="deleting"
...
const SCOPES = [
  { key: 'links', label: 'Links', options: ['read', 'write'] },
  { key: 'stats', label: 'Stats', options: ['read', 'none'] },
];
...
const onFilterChange = () => {
  ...
  searchTimer = setTimeout(() => store.fetchKeys({ search: search.value, status: statusFilter.value }), 350);
};

const goPage = (p) => store.fetchKeys({ search: search.value, status: statusFilter.value, page: p });
...
const handleSave = async () => {
  if (!form.keyName.trim()) { modalError.value = 'Key name is required'; return; }
  saving.value = true;
  modalError.value = '';
  try {
    if (editing.value) {
